...
public class ActivityClassGenerator {
	...
	public static void createActivityDex(String superClassName,
			Class<?> targetClass, File saveTo, String pluginId, String pkgName)
			throws IOException {
		byte[] dex = createActivityDex(superClassName, targetClass,
				pluginId, pkgName);
	...
	public static <S, D extends S> byte[] createActivityDex(
			final String superClassName, final Class<?> targetClass,
			final String pluginId, String pkgName) {

		DexMaker dexMaker = new DexMaker();

		TypeId<D> generatedType = TypeId.get('L' + targetClass.getName().replace('.',
                '/') + ';');
		...
		declareLifeCyleMethod(dexMaker, generatedType, superType, "overridePendingTransition");
