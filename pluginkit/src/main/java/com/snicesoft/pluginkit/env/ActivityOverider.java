...
import com.snicesoft.pluginkit.PluginActivityLifeCycleCallback;
...
public class ActivityOverider {
    ...
    private static void setPluginIntent(Intent intent, PlugInfo plugin,
        ...
        ComponentName compname = new ComponentName(mgr.getContext(), Globals.getPluginTargetActivity());
        intent.setComponent(compname)
                .putExtra(Globals.PLUGIN_ID, pluginId)
                .putExtra(Globals.PLUGIN_ACTIVITY, actName);
    }

    public static File getPluginBaseDir(String pluginId) {
    ...
    public static File getPluginLibDir(String pluginId) {
        File folder = new File(getPluginBaseDir(pluginId) + "/lib/");
        return folder;
    }

    public static File getPorxyActivityDexPath(String pluginId, String activity) {
    ...
    private static void createProxyDex(PlugInfo plugin, String activity, File saveDir,
        ...
        try {
            ...
            ActivityClassGenerator.createActivityDex(activity, Globals.getPluginTargetActivity(), saveDir, plugin.getId(), pkgName);
        } catch (Throwable e) {
    ...
    public static Object[] overrideAttachBaseContext(final String pluginId, final Activity fromAct, Context base) {
        ...
        PluginActivityWrapper actWrapper = new PluginActivityWrapper(base, plugin.appWrapper);
    ...
    private static void changeActivityInfo(Context activity) {
        ...
        if (activity.getClass() != Globals.getPluginTargetActivity()) {
...
//		}
    ...
    public static void callback_overridePendingTransition(String pluginId, Activity fromAct) {
        Log.d(tag, "callback_overridePendingTransition(act=" + fromAct.getClass().getSuperclass().getName() + ")");
        PluginActivityLifeCycleCallback callback = PluginManager.getInstance()
                .getPluginActivityLifeCycleCallback();
        if (callback != null) {
            callback.overridePendingTransition(pluginId, fromAct);
        }
    }
}
